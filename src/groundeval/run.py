...
def _merge_with_defaults(main_cfg: dict) -> dict:
    ...
    if not defaults_path.exists():
        logger.warning(f'  Defaults file not found at {defaults_path}; using main config as-is.')
...
def _is_framework_config(cfg: dict) -> bool:
    ...
    return framework in {'crewai', 'maf', 'langgraph'}
...
def _validate_config(cfg: dict, *, mode: str = 'task') -> None:
    ...
    if require_corpus and not any_fixture:
        ...
        if not art_path.exists():
            raise FileNotFoundError(f"Artifacts directory '{artifacts_dir}' does not exist. Set artifacts_dir in your config or create the directory.")
        artifacts = list(art_path.rglob('*.json'))
        if not artifacts:
            raise FileNotFoundError(f"Artifacts directory '{artifacts_dir}' contains no JSON files. Add seed artifact files before running.")
...
def cmd_task(args) -> None:
    ...
    if ge_cfg.get('generated_from_observation') and not ge_cfg.get('reviewed'):
        if not getattr(args, 'allow_draft_config', False):
            logger.warning('This config was generated from observation and has not been marked reviewed.')
            logger.warning('Run: groundeval validate --config {config_path} --mark-reviewed')
    ...
    artifacts_dir = cfg.get('artifacts_dir') or (contracts[0].artifacts_dir if contracts else './data')
    ...
    results = run_all_tasks(contracts=contracts, agent_fn=agent_fn, artifacts_dir=artifacts_dir, policy=policy, max_steps=args.max_steps)
    ...
    with open(out_path, 'w') as f:
        json.dump({'meta': {'model': resolved_model, 'n_tasks': len(results), 'evaluation_mode': 'task_contract'}, 'summary': summary}, f, indent=2)
    logger.info(f'Task results written to {out_path}')
    logger.info(f"Overall - counterfactual={summary['counterfactual_score']:.3f}  silence={summary['silence_score']:.3f}  perspective={summary['perspective_score']:.3f}  overall={summary['overall_score']:.3f}  accuracy={summary['accuracy']:.3f}")
    logger.info(f"Total violations: {summary['total_violations']}")
    for tr in results:
        logger.info(f"  {tr.task_name}: counterfactual={tr.counterfactual_score:.3f} silence={tr.silence_score:.3f} perspective={tr.perspective_score:.3f} overall={tr.overall_score:.3f}")


def _write_basic_observation_outputs(output_dir: str | Path, observed, stamp: str) -> None:
    ...
    diagram_tmp_path = write_observe_diagram_pdf(output_dir=out, observed=observed, direction='TD')
    ...
    if observed.framework_extra:
        from .framework_adapters.framework_observation import ObservedRun as FrameworkObservedRun
        framework_run = FrameworkObservedRun.from_dict(observed.framework_extra)
        rich_json_path = out / _timestamped_name(f'observed_run_{observed.framework}.json', stamp)
        with open(rich_json_path, 'w') as f:
            json.dump(framework_run.to_dict(), f, indent=2, default=str)
        logger.info(f'{observed.framework} rich observed run written to {rich_json_path}')
        ...
        if observed.framework == 'langgraph':
            ...
            rich_report_path = out / _timestamped_name('observe_report_langgraph.md', stamp)
            with open(rich_report_path, 'w') as f:
                f.write(generate_langgraph_report(framework_run))
            logger.info(f'LangGraph rich observation report written to {rich_report_path}')
...
def cmd_observe(args) -> None:
    from .observe import observe_agent, DraftGenerator, write_draft_output, score_observed_run
    ...
    observed = observe_agent(framework=args.framework, class_path=args.agent_class, max_steps=args.max_steps)
    logger.info(f'Observed run complete: {len(observed.tool_calls)} tool calls recorded')
    ...
    if score:
        ...
        if ge_cfg.get('generated_from_observation') and not ge_cfg.get('reviewed'):
            if not allow_draft_config:
                raise SystemExit(f'ERROR: This config was generated from observation and has not been marked reviewed. Run: groundeval validate --config {config_path} --mark-reviewed, or pass --allow-draft-config.')
        _validate_config(cfg, mode='observe_score')
        _write_basic_observation_outputs(output_dir, observed, stamp)
        _results, payload = score_observed_run(observed=observed, cfg=cfg, config_path=config_path)
        out = Path(output_dir)
        out.mkdir(parents=True, exist_ok=True)
    ...
    logger.info(f'  2. Mark reviewed: groundeval validate --config {output_dir}/draft_config/config.yaml --mark-reviewed')
    logger.info(f'  3. Score: groundeval observe --framework {args.framework} --agent-class {args.agent_class} --config {output_dir}/draft_config/config.yaml --score')
...
def _build_agent_fn(cfg: dict, contracts: list | None = None) -> Any:
    ...
    framework = str(agent_cfg.get('framework', '') or '').lower()
    if framework in {'crewai', 'maf', 'langgraph'}:
        raise SystemExit(f'{framework.upper()} framework agents are scored through observe --score, not task. Run: groundeval observe --framework {framework} --agent-class <path> --config <reviewed config.yaml> --score')
...
def main():
    ...
    logging.basicConfig(level=logging.INFO, force=True, format='%(asctime)s %(levelname)s %(name)s - %(message)s', datefmt='%H:%M:%S', handlers=[logging.FileHandler(str(log_file), mode='a'), logging.StreamHandler()])
    logger.info(f'Logging to {log_file}')
    parser = argparse.ArgumentParser(prog='groundeval', description='Deterministic agentic evaluation framework')
    sub = parser.add_subparsers(dest='command', required=True)
    task_parser = sub.add_parser('task', help='Run task-contract evaluation (no event log needed)')
    task_parser.add_argument('--config', required=True, help='Path to config.yaml with task_contracts')
    task_parser.add_argument('--model', default='claude-sonnet-4-6')
    task_parser.add_argument('--max-steps', type=int, default=10)
    task_parser.add_argument('--allow-draft-config', action='store_true', help='Allow running evaluation with an unreviewed draft config')
    observe_parser = sub.add_parser('observe', help='Observe an existing agent and generate draft eval config')
    observe_parser.add_argument('--framework', required=True, help='Agent framework (e.g. crewai)')
    observe_parser.add_argument('--agent-class', required=True, help='Dotted Python path to the agent class')
    observe_parser.add_argument('--no-draft', action='store_true', help='Skip draft config generation')
    observe_parser.add_argument('--config', help='Reviewed config.yaml to score the observed run against')
    observe_parser.add_argument('--score', action='store_true', help='Score this observed run against --config')
    observe_parser.add_argument('--allow-draft-config', action='store_true', help='Allow scoring with an unreviewed draft config')
    observe_parser.add_argument('--draft-mode', choices=['conservative', 'standard', 'aggressive'], default='standard', help='How much inference to apply when generating draft config')
    observe_parser.add_argument('--output', default='./eval_output', help='Output directory')
    observe_parser.add_argument('--max-steps', type=int, default=10)
    draft_parser = sub.add_parser('draft', help='Generate draft config from an existing observed run')
    draft_parser.add_argument('--from-run', required=True, help='Path to observed_run.json')
    draft_parser.add_argument('--draft-mode', choices=['conservative', 'standard', 'aggressive'], default='standard')
    draft_parser.add_argument('--output', help='Output directory')
    compare_parser = sub.add_parser('compare', help='Compare two GroundEval JSON outputs')
    ...
    val = sub.add_parser('validate', help='Validate config + artifacts without running tasks')
    val.add_argument('--config', required=True, help='Path to config.yaml')
    val.add_argument('--mark-reviewed', action='store_true', help='Mark a draft config as reviewed')
