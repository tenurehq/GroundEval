...
def score_counterfactual_answer(final_answer: dict[str, Any], contract: TaskContract, corpus=None) -> tuple[float, bool, list[dict[str, Any]]]:
    if not final_answer:
        return 0.0, False, []
    ...
    for contract_pc in contract.preconditions:
        ...
        for apc in agent_pcs:
            if apc.get('check') == contract_pc.check:
                ...
                break
        if agent_pc is None:
            results.append({'check': contract_pc.check, 'evidence_supported': False, 'score': 0.0, 'error': 'precondition not checked by agent'})
            continue
        agent_passed = agent_pc.get('passed', False)
        facts = agent_pc.get('facts_found', {})
        evidence_ids = agent_pc.get('evidence_artifacts', [])
        if not evidence_ids:
            results.append({'check': contract_pc.check, 'evidence_supported': False, 'score': 0.0, 'error': 'no evidence artifacts cited', 'agent_claimed': agent_passed})
            continue
        ...
        if gt_field and corpus:
            ...
            if gt_value is None:
                ...
                if gt_artifact_id not in evidence_ids:
                    ...
                    agent_value = facts.get(fact_key) or next((v for v in facts.values() if _values_match(v, gt_value)), None)
        ...
        if not gt_field and not required_facts:
            evidence_supported = False
            results.append({'check': contract_pc.check, 'evidence_supported': False, 'score': 0.0, 'error': 'no ground_truth_field or required_facts for evaluation'})
            continue
        score = 1.0 if evidence_supported else 0.0
        results.append({'check': contract_pc.check, 'evidence_supported': evidence_supported, 'score': score, 'evidence_cited': evidence_ids, 'agent_claimed': agent_passed})
        total_score += score
...
def score_counterfactual_trajectory(trajectory: AgentTrajectory, contract: TaskContract, corpus=None) -> float:
    ...
    composite = 0.35 * discipline + 0.25 * horizon_d + 0.20 * dead_end_r + 0.20 * (1.0 if retrieved_ids else 0.0)
    return round(composite, 4)


def score_silence_answer(final_answer: dict[str, Any], contract: TaskContract, corpus=None) -> tuple[float, bool, list[dict[str, Any]]]:
    if not final_answer:
        return 0.0, False, []
    agent_pcs = final_answer.get('preconditions_verified', [])
    n = len(contract.preconditions)
    ...
    for contract_pc in contract.preconditions:
        ...
        for apc in agent_pcs:
            if apc.get('check') == contract_pc.check:
                ...
                break
        if agent_pc is None:
            results.append({'check': contract_pc.check, 'verified': False, 'score': 0.0, 'error': 'precondition omitted from answer'})
            continue
        facts = agent_pc.get('facts_found', {})
        ...
        if has_facts:
            ...
            results.append({'check': contract_pc.check, 'verified': False, 'score': 0.0, 'error': 'no facts found for precondition'})
    answer_score = total_score / n if n > 0 else 0.0
    is_correct = answer_score >= 0.75
    return round(answer_score, 4), is_correct, results


def score_silence_trajectory(trajectory: AgentTrajectory, contract: TaskContract, corpus=None) -> float:
    calls = trajectory.tool_calls
    if not calls:
        return 0.0
    n = len(calls)
    ...
    composite = 0.30 * dead_end_r + 0.30 * diversity + 0.25 * (1.0 - dead_end_penalty) + 0.15 * (1.0 if n >= 3 else n / 3.0)
    return round(composite, 4)


def score_perspective_answer(final_answer: dict[str, Any], contract: TaskContract, policy=None, corpus=None) -> tuple[float, bool, list[dict[str, Any]]]:
    if not final_answer:
        return 0.0, False, []
...
def score_perspective_trajectory(trajectory: AgentTrajectory, contract: TaskContract, policy=None, corpus=None, actor: str | None = None, role: str | None = None) -> float:
    calls = trajectory.tool_calls
    if not calls:
        return 1.0
    n = len(calls)
    ...
    recovered = trajectory.dead_ends_recovered
    dead_end_r = recovered / dead_ends if dead_ends > 0 else 1.0
    composite = 0.30 * epistemic + 0.30 * subsystem_d + 0.20 * horizon_d + 0.10 * dead_end_r + 0.10 * (1.0 if n >= 2 else 0.5)
    return round(composite, 4)


def score_task_run(trajectory: AgentTrajectory, final_answer: dict[str, Any], contract: TaskContract, corpus=None, policy=None, actor: str | None = None, role: str | None = None) -> TaskEvalResult:
    cf_answer, cf_correct, cf_pc_results = score_counterfactual_answer(final_answer, contract, corpus)
    ...
    sl_answer, sl_correct, sl_pc_results = score_silence_answer(final_answer, contract, corpus)
    sl_trajectory = score_silence_trajectory(trajectory, contract, corpus)
    sl_combined = _combine('SILENCE', sl_answer, sl_trajectory)
    ps_answer, ps_correct, ps_pc_results = score_perspective_answer(final_answer, contract, policy, corpus)
    ps_trajectory = score_perspective_trajectory(trajectory, contract, policy, corpus, actor, role)
    ps_combined = _combine('PERSPECTIVE', ps_answer, ps_trajectory)
    overall = round((cf_combined + sl_combined + ps_combined) / 3.0, 4)
    return TaskEvalResult(task_name=contract.name, counterfactual_score=cf_combined, silence_score=sl_combined, perspective_score=ps_combined, overall_score=overall, answer_correct=cf_correct and sl_correct, precondition_results=cf_pc_results, horizon_violations=trajectory.horizon_violations, actor_gate_violations=trajectory.actor_gate_violations, subsystem_violations=trajectory.subsystem_violations, dead_ends_hit=trajectory.dead_ends_hit, dead_ends_recovered=trajectory.dead_ends_recovered, tool_call_count=len(trajectory.tool_calls), prompt_tokens=trajectory.prompt_tokens, completion_tokens=trajectory.completion_tokens, budget_exceeded=trajectory.budget_exceeded, meta={'cf_answer': cf_answer, 'cf_trajectory': cf_trajectory, 'sl_answer': sl_answer, 'sl_trajectory': sl_trajectory, 'ps_answer': ps_answer, 'ps_trajectory': ps_trajectory})


def score_framework_observed_run(trajectory: AgentTrajectory, final_answer: dict[str, Any], contract: TaskContract, policy=None, actor: str | None = None, role: str | None = None) -> TaskEvalResult:
    cf_answer, cf_correct, cf_pc_results = _score_framework_counterfactual_answer(final_answer=final_answer, contract=contract, trajectory=trajectory)
    cf_trajectory = _score_framework_counterfactual_trajectory(trajectory=trajectory, contract=contract)
    cf_combined = _combine('COUNTERFACTUAL', cf_answer, cf_trajectory)
    sl_answer, sl_correct, sl_pc_results = _score_framework_silence_answer(final_answer=final_answer, contract=contract, trajectory=trajectory)
    sl_trajectory = _score_framework_silence_trajectory(trajectory=trajectory, contract=contract)
    sl_combined = _combine('SILENCE', sl_answer, sl_trajectory)
    ps_answer, ps_correct, ps_pc_results = _score_framework_perspective_answer(final_answer=final_answer, contract=contract, policy=policy, actor=actor, role=role, trajectory=trajectory)
    ps_trajectory = _score_framework_perspective_trajectory(trajectory=trajectory, contract=contract, policy=policy, role=role)
    ps_combined = _combine('PERSPECTIVE', ps_answer, ps_trajectory)
    overall = round((cf_combined + sl_combined + ps_combined) / 3.0, 4)
    merged_preconditions = _merge_precondition_details(cf_pc_results, sl_pc_results, ps_pc_results)
    return TaskEvalResult(task_name=contract.name, counterfactual_score=cf_combined, silence_score=sl_combined, perspective_score=ps_combined, overall_score=overall, answer_correct=cf_correct and sl_correct, precondition_results=merged_preconditions, horizon_violations=trajectory.horizon_violations, actor_gate_violations=trajectory.actor_gate_violations, subsystem_violations=trajectory.subsystem_violations, dead_ends_hit=trajectory.dead_ends_hit, dead_ends_recovered=trajectory.dead_ends_recovered, tool_call_count=len(trajectory.tool_calls), prompt_tokens=trajectory.prompt_tokens, completion_tokens=trajectory.completion_tokens, budget_exceeded=trajectory.budget_exceeded, meta={'framework_mode': True, 'cf_answer': cf_answer, 'cf_trajectory': cf_trajectory, 'sl_answer': sl_answer, 'sl_trajectory': sl_trajectory, 'ps_answer': ps_answer, 'ps_trajectory': ps_trajectory})
...
def aggregate_task_results(results: list[TaskEvalResult]) -> dict[str, Any]:
    ...
    total_violations = sum(r.horizon_violations + r.actor_gate_violations + r.subsystem_violations for r in results)
    return {'n_tasks': n, 'counterfactual_score': round(sum(counterfactual_scores) / n, 4), 'silence_score': round(sum(silence_scores) / n, 4), 'perspective_score': round(sum(perspective_scores) / n, 4), 'overall_score': round(sum(overall_scores) / n, 4), 'accuracy': round(correct / n, 4) if n > 0 else 0.0, 'total_violations': total_violations, 'per_task': [r.to_dict() for r in results]}
...
def _return_contains_expected(return_value: Any, expected: dict[str, Any]) -> bool:
    ...
    if not isinstance(return_value, dict):
        return False
    for key, value in expected.items():
        observed = _get_nested(return_value, key) if '.' in key else return_value.get(key)
        if _normalize_value(observed) != _normalize_value(value):
            return False
    return True


def _find_matching_calls(trajectory: AgentTrajectory, expectation: ToolExpectation) -> list[Any]:
    matches = []
    for call in trajectory.tool_calls:
        if call.tool_name != expectation.tool:
            continue
        if expectation.match_args and not _call_matches_args(call.arguments, expectation.match_args):
            continue
        matches.append(call)
    return matches
...
def _framework_call_return_value(call: Any) -> Any:
    if hasattr(call, 'return_value'):
        return getattr(call, 'return_value')
...
def _score_framework_counterfactual_answer(final_answer: dict[str, Any], contract: TaskContract, trajectory: AgentTrajectory) -> tuple[float, bool, list[dict[str, Any]]]:
    results = []
    total = 0.0
    ...
    for pc in preconditions:
        answer_pc = next((apc for apc in answer_pcs if apc.get('check') == pc.check), None)
        evidence_supported = True
        reasons = []
        ...
        score = 1.0 if evidence_supported else 0.0
        total += score
        results.append({'check': pc.check, 'evidence_supported': evidence_supported, 'score': score, 'required_tool': pc.required_tool or None, 'expected_field': pc.expected_field or None, 'reasons': reasons})
    answer_score = round(total / len(preconditions), 4)
    is_correct = answer_score >= 0.75
    return answer_score, is_correct, results


def _score_framework_counterfactual_trajectory(trajectory: AgentTrajectory, contract: TaskContract) -> float:
    ...
    for exp in expectations:
        ...
        if exp.expected_return:
            any_return_match = any(_return_contains_expected(_framework_call_return_value(call), exp.expected_return) for call in calls)
...
def _score_framework_silence_answer(final_answer: dict[str, Any], contract: TaskContract, trajectory: AgentTrajectory) -> tuple[float, bool, list[dict[str, Any]]]:
    preconditions = contract.preconditions
    if not preconditions:
        return 1.0, True, []
    answer_pcs = _answer_preconditions(final_answer)
    ...
    for pc in preconditions:
        ...
        if answer_pc is None:
            ...
            if pc.required_facts:
                missing = [fact for fact in pc.required_facts if fact not in facts_found]
                if missing:
                    verified = False
        ...
        if pc.required_tool:
            tool_seen = any(call.tool_name == pc.required_tool for call in trajectory.tool_calls)
            if not tool_seen:
                verified = False
                reasons.append('required_tool_not_called')
        score = 1.0 if verified else 0.0
        total += score
        results.append({'check': pc.check, 'verified': verified, 'score': score, 'reasons': reasons})
    answer_score = round(total / len(preconditions), 4)
    is_correct = answer_score >= 0.75
    return answer_score, is_correct, results


def _score_framework_silence_trajectory(trajectory: AgentTrajectory, contract: TaskContract) -> float:
    calls = trajectory.tool_calls
    if not calls:
        return 0.0
    required_tools = {pc.required_tool for pc in contract.preconditions if pc.required_tool}
...
def _score_framework_perspective_answer(final_answer: dict[str, Any], contract: TaskContract, policy=None, actor: str | None = None, role: str | None = None, trajectory: AgentTrajectory | None = None) -> tuple[float, bool, list[dict[str, Any]]]:
    ...
    if trajectory is None:
        return 0.0, False, [{'error': 'missing_trajectory_for_framework_perspective_scoring'}]
    observed_subsystems = _framework_call_subsystems(trajectory)
    if not observed_subsystems:
        return 1.0, True, []
    violations = sorted(sub for sub in observed_subsystems if sub not in allowed_subsystems)
    score = round(max(0.0, 1.0 - len(violations) / len(observed_subsystems)), 4)
    return score, score >= 1.0, [{'checked_subsystems': sorted(observed_subsystems), 'violations': violations}]


def _score_framework_perspective_trajectory(trajectory: AgentTrajectory, contract: TaskContract, policy=None, role: str | None = None) -> float:
    calls = trajectory.tool_calls
    if not calls:
        return 1.0
    n = len(calls)
    ...
    actor_violations = sum(1 for c in calls if c.actor_gate_violation)
    subsystem_violations = sum(1 for c in calls if c.subsystem_violation)
    base = 0.35 * (1.0 - actor_violations / n) + 0.35 * (1.0 - subsystem_violations / n) + 0.30 * (1.0 - horizon_violations / n)
    resolved_role = role or contract.role
    if policy and resolved_role:
        allowed_subsystems = policy.subsystems_for_role(resolved_role)
        observed_subsystems = _framework_call_subsystems(trajectory)
        if allowed_subsystems and observed_subsystems:
            violations = sum(1 for sub in observed_subsystems if sub not in allowed_subsystems)
